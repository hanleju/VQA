...
python ./attack/loss.py -c ./cfg/cocoqa/Res_Bert_Lora_dp.yaml -w ./checkpoints/cocoqa/Res_Bert_Lora_dp/best_model.pth --threshold 1.0
...
def run_threshold_sweep(model, train_loader, test_loader, device, model_type, result_dir, weight_name):
    ...
    thresholds = np.arange(0.0, 3.0, 0.1)
    results = []
    ...
    # Loss는 낮을수록 member이므로, score를 반전 (-loss 사용)
    ...
    bars = ax.bar(x, accuracies, width=0.08, alpha=0.8, color='coral', edgecolor='black')
    ax.set_xlabel('Threshold', fontsize=12, fontweight='bold')
    ax.set_ylabel('Attack Accuracy', fontsize=12, fontweight='bold')
    ax.set_title('Loss-based MIA: Attack Accuracy vs Threshold', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def main():
    ...
    save_privacy_metrics(
        ...
        metric_name="Loss", tpr_at_low_fpr=tpr_at_low_fpr
    )
